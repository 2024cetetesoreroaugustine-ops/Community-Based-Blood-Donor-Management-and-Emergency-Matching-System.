...
if ($method === 'GET') {
    ...
    FROM NOTIFICATION
    WHERE USERS_UserID = ? AND status = 'Unread'
...
if ($method === 'PUT') {
        ...
        try {
            if ($notificationId) {
                $updNotif = $pdo->prepare("UPDATE NOTIFICATION
                    ...
                    WHERE NotificationID = ? AND USERS_UserID = ?");
                $updNotif->execute([$notificationId, $donorUserId]);
            } elseif ($reqId) {
                $updNotif = $pdo->prepare("UPDATE NOTIFICATION
                    SET status = 'Responded'
    ...
    if ($action === 'clear_all') {
        ...
        $stmt = $pdo->prepare("UPDATE NOTIFICATION
            SET status = 'Dismissed'
            WHERE USERS_UserID = ? AND status <> 'Dismissed'");
        $stmt->execute([$userId]);

        json_response(true, 'All notifications cleared.', [
            'user_id' => $userId,
